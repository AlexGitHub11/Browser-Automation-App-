...
class App:
    def __init__(self, root):
        ...
        self.entry_current_addr = tk.Entry(self.form_frame)
        self.entry_current_addr.grid(row=2, column=1, sticky="ew")
        tk.Label(self.form_frame, text="Permanent Address").grid(row=3, column=0, sticky="w")
        self.entry_permanent_addr = tk.Entry(self.form_frame)
        self.entry_permanent_addr.grid(row=3, column=1, sticky="ew")
    ...
    def submit_data(self):
        """ Get user input and then pass to WebAutomation class methods """
        username = self.entry_username.get()
        password = self.entry_password.get()
        fullname = self.entry_fullname.get()
        email = self.entry_email.get()
        current_address = self.entry_current_addr.get()
        permanent_address = self.entry_permanent_addr.get()

        self.web_automation = WebAutomation()
        self.web_automation.login(username=username, password=password)
        self.web_automation.fill_form(fullname=fullname, email=email,
